...
4. **Claude Code prompt pattern** แนะนำ:
   > "อ้างอิง DEVELOPMENT_PLAN.md — ทำ Phase X, Task Y: [ชื่อ task] ให้เสร็จตาม Acceptance Criteria ที่ระบุ"
...
**Session prompt:** `"ทำ Phase 0 ใน DEVELOPMENT_PLAN.md ให้ครบ"`
...
**What to do:**
- Frontend: เมื่อ `card-due` หรือ `card-start` ว่าง ให้ส่ง `due: null` / `start: null` ใน payload เสมอ
...
**Session prompt:** `"ทำ Phase 1 ใน DEVELOPMENT_PLAN.md — สร้าง backend Review Queue APIs"`
...
**Session prompt:** `"ทำ Phase 2 ใน DEVELOPMENT_PLAN.md — สร้าง Review Queue UI"`
...
**Acceptance Criteria:**
- [ ] OAuth flow เพิ่ม Tasks scope โดยไม่ break Calendar
- [ ] `GET /api/google-tasks/today` คืน task list
- [ ] `PUT .../complete` → task หายจาก incomplete list
...
**Acceptance Criteria:**
- [ ] Google Tasks connected → แสดง tasks จริง
- [ ] Google Tasks ไม่ connected → แสดงปุ่ม "Connect Google Tasks" → เปิด OAuth
- [ ] Checkbox ✓ → call PUT mark complete → task หายจาก list
- [ ] Trello cards due today/tomorrow ดึงจาก `S.allCardsCache`
- [ ] Quick add → สร้าง Google Task ใหม่
...
**Session prompt:** `"ทำ Phase 5 ใน DEVELOPMENT_PLAN.md — Today Dashboard enhanced"`
...
**Acceptance Criteria:**
- [ ] มี pending review tasks → stat card แสดงตัวเลขจริง
- [ ] Section "Pending Review" แสดง task cards (title, meeting source, diff badge)
- [ ] คลิก task → navigate ไป Review Queue และ expand session นั้น
...
**Acceptance Criteria:**
- [ ] GCal connected → แสดง events วันนี้
- [ ] GCal ไม่ connected → section hidden
- [ ] Event แสดง: title, time range, ปุ่ม Open in Calendar
...
**Acceptance Criteria:**
- [ ] Type task name + Enter → modal/popup ให้เลือก board/list โผล่
- [ ] เลือก board/list → create card → Today page refresh
- [ ] Card ใหม่โชว์ใน section ที่ถูกต้อง (due today / no due)
...
**Session prompt:** `"ทำ Phase 6 ใน DEVELOPMENT_PLAN.md — Hardening & Polish"`
...
- [ ] Session ที่มี 5 tasks บน board เดียวกัน → Trello `getLists` ถูกเรียกแค่ 1 ครั้ง (ไม่ใช่ 5 ครั้ง)
...
**Acceptance Criteria:**
- [ ] Tasks ที่ `diffStatus: "create_new"` มี `matchReason: ""` ใน JSON (ไม่ใช่ internal message)
...
**Acceptance Criteria:**
- [ ] Processed task ที่เคยเป็น `update_existing` → แสดง matchReason เล็กๆ ใต้ status label
- [ ] Processed task ที่เป็น `create_new` → ไม่แสดงอะไรเพิ่ม (เหมือนเดิม)
