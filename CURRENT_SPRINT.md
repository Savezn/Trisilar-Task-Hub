...
| UIV2-01 | UI V2 route-by-route implementation | S0 and S1 merged; S2 All Tasks inbox QA pass on `codex/v06-s2-all-tasks-inbox` with dense scan strip, saved-filter state, hidden-board notice, owner/source/context/next-action cues, clear-filter empty state, and screenshot evidence | PM review / merge |
...
| V0.6 UI V2 route implementation | Frontend Dev / UX Owner / QA | S0 shell/navigation and S1 Today are merged. S2 All Tasks has QA pass evidence on `codex/v06-s2-all-tasks-inbox`; PM merge review is next, then continue S3 Review Queue. |
...
- S0 and S1 are merged. Continue route-by-route with S2 All Tasks PM merge review, then S3 Review Queue.
