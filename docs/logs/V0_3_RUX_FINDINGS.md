...
**Status:** Active - V0.3-RUX-06 PM accepted / dev integration candidate in progress
**Owner:** PM / UX / QA
**Created:** 2026-05-14
...
| RUX-005 | 2026-05-14 | All core routes | Regression risk | P1 high friction | Browser QA evidence was repeated manually across RUX tasks without one repeatable command. | Release confidence depends on re-running the same route matrix without relying on memory, production credentials, or ad hoc browser scripts. | Fix verification: RED missing-script failure observed, then `npm.cmd run verify:rux-browser-regression` passed. The command starts a temporary local server, uses controlled fixtures, checks `/today`, `/review`, `/all`, `/boards`, `/calendar`, `/planner`, `/okr`, `/focus`, `/settings`, and `/docs` across desktop/mobile, captures console/page errors and overflow, and requires no production secrets. PM accepted at `0af9417`. | QA / Frontend / Dev | `V0.3-RUX-05` | A repeatable browser regression command covers core routes with controlled fixtures, desktop/mobile viewports, console/page error capture, overflow checks, and relevant RUX assertions. | PM Accepted |
| RUX-006 | 2026-05-14 | Release / All routes | Release blocker | P0 blocker before release | V0.3 needed one artifact that a PM, QA, Integration Owner, and Runtime Owner can use before a future `dev -> main` decision. | Without a single checklist, release confidence would depend on scattered branch, QA, runtime, Paperclip, browser, and rollback evidence. | Fix verification: `docs/plans/VERSION_0_3_RUX_06_RELEASE_CHECKLIST_DEV_MAIN.md` now lists source branch, stacked operating-model dependency, accepted V0.3 evidence, integration order, verification commands, browser route matrix, Paperclip/Review Queue gate, runtime/access gate, rollback/hold plan, and PM decision block. Docs-only verification uses `git diff --check`. PM accepted at `df29307`; operating-model prerequisite is merged into `dev@ed9fae0`, and integrated candidate QA is now the active gate. | PM / QA / Integration / Runtime | `V0.3-RUX-06` | PM can decide future `dev -> main` promotion from one checklist artifact while preserving V0.3/W3 boundaries, runtime/secret safety, and no-merge/no-deploy scope for this phase. | PM Accepted |
...
| RUX-B13 | 2026-05-14 | `V0.3-RUX-06` Release Checklist for `dev -> main` | Codex PM / QA / Integration / Runtime | PM Accepted / dev integration candidate in progress | Drafted one release checklist artifact covering source branch, stacked operating-model dependency, accepted V0.3 evidence, integration order, verification commands, browser route matrix, Paperclip/Review Queue gate, runtime/access gate, rollback/hold plan, and PM decision format. PM accepted at `df29307` and routed at `5a90cc7`. Docs-only scope preserved: no deploy, runtime flag change, live Paperclip enablement, secret exposure, or W3/V0.3 cross-merge. The operating-model prerequisite is now merged into `dev@ed9fae0`; current gate is integrated candidate QA on latest `dev`. |
...
| 2026-05-14 | Accepted `V0.3-RUX-05`; routed `V0.3-RUX-06` Release Checklist for dev -> main | Codex PM |
...
| 2026-05-14 | Accepted `V0.3-RUX-06`; marked `RUX-006` as PM Accepted and routed next to Integration Owner for operating-model prerequisite handling | Codex PM |
