...
| 2026-05-14 | Plan `V0.2-W3-03` controlled Paperclip live enablement policy | W3 was merged into `dev` at `a89c26a`, but runtime enablement remains separate. PM policy requires explicit enablement criteria, named Runtime Owner, Paperclip Owner and QA owner, rollback actions for feature flag/sender/secret/Cloudflare service token, monitoring/audit expectations without secrets, and preserved Review Queue human gate before any standing `PAPERCLIP_WEBHOOK_ENABLED=true` window. | Codex PM |
