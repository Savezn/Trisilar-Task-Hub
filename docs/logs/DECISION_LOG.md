...
| 2026-05-17 | Adopt lightweight PM task role routing template | PM-routed tasks now use `Primary role`, `Next role`, `Optional support role`, `Boundary`, and `Route if` so every task has one clear owner, expected handoff, and route condition without creating a large role matrix or allowing support roles to expand authority. | Codex PM / Documentation Workflow Owner |
