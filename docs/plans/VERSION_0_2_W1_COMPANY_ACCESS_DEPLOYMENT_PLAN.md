...
**Status:** Active - `V0.2-W1-05` accepted as random ngrok URL manual demo only; next W1 path is DigitalOcean hosted dev/demo with Cloudflare front door and Access
**Version:** V0.2
...
PM now confirmed a Cloudflare-managed domain exists and is evaluating a lower-cost hosted dev/demo runtime on DigitalOcean. The updated W1 direction is to keep Cloudflare as the DNS/security front door and use a DigitalOcean Droplet as the preferred always-on dev/demo runtime candidate. Render remains a previously approved managed paid target, Railway remains the managed alternate, but DigitalOcean is now the next W1 runtime path to evaluate because it can host long-running Node services with file-backed state at low monthly cost.

Paperclip is currently running on localhost on Noffy's machine. That means Task Hub can move to a stable hosted URL before Paperclip does, but live Paperclip integration remains blocked until Paperclip has a reachable stable endpoint or Noffy exposes Paperclip through its own Cloudflare Tunnel/hosted runtime.

Current W1 decision:

- Keep ngrok + temporary Basic Auth as the accepted short manual demo path only.
- Use DigitalOcean Droplet + Cloudflare front door as the next hosted dev/demo runtime candidate.
...
- Treat Paperclip localhost on Noffy's machine as an external runtime dependency until Paperclip gets a stable reachable hostname.
...
- DigitalOcean hosted dev/demo runtime planning and setup.
...
V0.2-W1-01..V0.2-W1-03 repo readiness done
    ...
    -> V0.2-W1-08 DigitalOcean hosted dev/demo runtime setup
...
- Runtime setup may use local machine and ngrok for the temporary demo. The next hosted path uses DigitalOcean Droplet plus Cloudflare DNS/Tunnel/Access. Secrets stay out of git.
...
- Paperclip currently runs on localhost on Noffy's machine; Task Hub cannot depend on that for stable integration until Paperclip is exposed by Noffy's Cloudflare Tunnel or moved to an always-on hosted runtime.
...
| `V0.2-W1-04` | `W1.3` | Accepted / amended | PM | Preview/runtime decision | Random ngrok accepted for manual demo; DigitalOcean + Cloudflare selected for next hosted dev/demo evaluation |
...
| `V0.2-W1-07` | `W1.6` | Pending | Dev / PM | Paperclip agent/API access prep | Service-token pattern documented; Paperclip localhost dependency and stable endpoint requirement recorded without W3 implementation |
| `V0.2-W1-08` | `W1.7` | Next candidate | PM / Dev / QA | DigitalOcean hosted dev/demo runtime | Droplet runtime, persistent `APP_DATA_DIR`, Cloudflare routing, `/healthz`, access gate, and non-destructive app load verified |
...
- Use DigitalOcean Droplet + Cloudflare as the next always-on dev/demo candidate.
...
- Record that Paperclip currently runs on localhost on Noffy's machine and is not reachable as a stable service unless Noffy exposes it through Cloudflare Tunnel or it moves to hosted runtime.
...
- Paperclip localhost dependency is recorded as a W3 runtime blocker.
- No new W3 implementation is introduced.

### V0.2-W1-08 - DigitalOcean Hosted Dev/Demo Runtime
...
- Use DigitalOcean Droplet as the preferred next hosted dev/demo runtime candidate.
...
- Create or confirm a dev-only DigitalOcean Droplet.
...
- Verify `/healthz`, authenticated app load, Basic Google/Trello non-destructive routes, and `APP_DATA_DIR` persistence.
- Decide whether Paperclip stays on Noffy's localhost behind its own tunnel or migrates later.
...
- Treat Paperclip localhost on Noffy's machine as a runtime blocker for live W3 integration unless it is exposed through a stable Cloudflare hostname or moved to hosted runtime.
...
| `V0.2-W1-07` | 1 | Alias W1.6; documentation/pattern only; records Paperclip localhost blocker; no live W3 behavior |
| `V0.2-W1-08` | 1-2 | Alias W1.7; DigitalOcean hosted dev/demo runtime setup and verification |
...
Context:
W1 repo deploy-readiness and dev deployment config are merged to `dev`. `V0.2-W1-05` random ngrok demo passed and remains accepted for short manual demo only. PM confirmed a Cloudflare-managed domain exists and is evaluating DigitalOcean Droplet as the next always-on hosted dev/demo runtime. Paperclip currently runs on localhost on Noffy's machine, so W3 live integration remains blocked until Paperclip has a stable reachable hostname or tunnel.
...
Steps:
1. Confirm the Cloudflare domain and desired hostnames, default `taskhub-dev.<domain>` and optional `paperclip-dev.<domain>`.
2. Confirm DigitalOcean account access and create or select one dev-only Droplet.
3. Pull the `dev` branch on the Droplet and configure dev-only `.env` values on the server only.
4. Configure persistent `APP_DATA_DIR`.
5. Configure `APP_BASE_URL` and `GOOGLE_REDIRECT_URI` for the Task Hub Cloudflare hostname.
6. Configure Cloudflare routing to the Droplet using Tunnel or proxied DNS.
...
9. Record that Paperclip is still localhost on Noffy's machine unless Noffy exposes it separately.
